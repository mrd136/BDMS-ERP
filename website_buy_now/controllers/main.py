    ...
    def check_cart_in_session(self):
        ...
        if request.website.is_public_user():
            ...
            try:
                ...
                if res_partner.save_user_cart_data:
            ...
            if kw.get('buy_now'):
                ...
                if not product_for_later:
                    ...
                    for lines in sale_order.order_line:
                        if not lines.is_delivery:
                            if product_for_later.get(lines.product_id.id):
                                product_for_later.update({lines.product_id.id : lines.product_uom_qty+product_for_later.get(lines.product_id.id)})
                            else:
                ...
                for f in self._get_mandatory_billing_fields():
                    if not sale_order.partner_id[f]:
                        return request.redirect('/shop/address?partner_id=%d' % sale_order.partner_id.id)
